...
const PersonalizationPanel: React.FC = () => {
 ...
 const handleToolbarMetricVisibilityToggle = (metricKey: BasicMetricKey) => {
  ...
  const handleToggleComprehensivePrompt = () => {
    ...
    // Mostra "Salvo!" por um breve período para dar feedback ao usuário.
    // A lógica de debounce no EditorContext já está a cuidar do salvamento real no Firestore.
    setTimeout(() => {
      setSaveStatus('saved');
      // Volta para o estado normal depois de um tempo
      setTimeout(() => setSaveStatus('idle'), 2000); 
    }, 700); // Um pequeno delay para a transição do estado
  };
    ...
    <div className={styles.personalizationContainer}>
      ...
      >
        ...
        <div className={styles.actionButtons}>
            {/* <<< MUDANÇA NO BOTÃO para mostrar o feedback >>> */}
            <Button onClick={handleSavePreferences} variant="primary" className={styles.confirmButton} disabled={saveStatus === 'saving'}>
              {saveStatus === 'idle' && 'Salvar Preferências'}
