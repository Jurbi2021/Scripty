...
if (percentage === 0) {
    ...
    level = 'Excelente';
  } else if (percentage <= thresholds.passiveVoicePercent) { // Usa o limiar do perfil
...
export const analyzeAdverbs = (words: string[], thresholds: ThresholdConfig): AdverbResult => {
  ...
  if (percentage <= thresholds.adverbPercent / 2) {
    feedback = 'Ótimo! Uso moderado de advérbios.';
    level = 'Excelente';
  } else if (percentage <= thresholds.adverbPercent) { // Usa o limiar do perfil
...
export const analyzeComplexSentences = (sentences: string[], thresholds: ThresholdConfig): ComplexSentencesResult => {
  ...
  if (percentage === 0) {
        ...
        level = 'Excelente';
    } else if (percentage <= thresholds.complexSentencePercent) { // Usa o limiar do perfil
...
export const analyzeLexicalDiversity = (words: string[], thresholds: ThresholdConfig): LexicalDiversityResult => {
    const totalWords = words.length;
    ...
    if (index < thresholds.shannonIndexLow) {
    feedback = `Baixa diversidade léxica (${index}). Use mais variedade de palavras.`;
    level = 'Ruim';
  } else if (index <= thresholds.shannonIndexHigh) {
...
export const performStyleAnalysis = (text: string, thresholds: ThresholdConfig): StyleAnalysisData => {
  const sentences = text.split(/[.!?]+/).filter(sentence => sentence.trim().length > 0);
  const words = text.split(/\s+/).filter(word => word.length > 0);

  // <<< MUDANÇA: Passa os 'thresholds' para as sub-funções >>>
  ...
  // As funções abaixo já eram baseadas em lógica interna, mas podem ser adaptadas se necessário
  const discourseConnectors = analyzeDiscourseConnectors(words); 
  const lexicalDiversity = analyzeLexicalDiversity(words, thresholds); // <<< PASSAR AQUI
